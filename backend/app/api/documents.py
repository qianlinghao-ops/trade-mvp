...
async def upload_document(
    ...
    """ファイルをアップロードしてOCR解析を実行"""
    ext = Path(file.filename).suffix.lower()
    if ext not in ALLOWED_EXTENSIONS:
        raise HTTPException(400, f"対応していないファイル形式です: {ext}")
...
async def generate_doc(
    ...
    try:
        ...
        raise HTTPException(500, f"書類生成エラー: {str(e)}")
...
async def generate_manual(body: dict, db: Session = Depends(get_db)):
    """手動入力データから書類を直接生成"""
    doc_type = body.get("doc_type", "po")
    data = body.get("data", {})

    doc = Document(
        doc_type=doc_type,
        status=DocumentStatus.completed,
        original_filename="手動入力",
        stored_filename="",
    ...
    try:
        generate_document(doc_type, data, gen_name)
        doc.generated_filename = gen_name
        db.commit()
    except Exception as e:
        raise HTTPException(500, f"書類生成エラー: {str(e)}")
...
async def download_document(doc_id: str, db: Session = Depends(get_db)):
    """生成済み書類をダウンロード"""
    doc = db.query(Document).filter(Document.id == doc_id).first()
    if not doc or not doc.generated_filename:
        raise HTTPException(404, "生成済み書類が見つかりません")
        ...
        "items": [
            {
                ...
                "has_generated": bool(d.generated_filename),
            }
            for d in docs
        ]
    }
