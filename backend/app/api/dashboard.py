...
async def get_dashboard(db: Session = Depends(get_db)):
    ...
    po_active = db.query(PurchaseOrder).filter(
    ...
    so_active = db.query(SalesOrder).filter(
        SalesOrder.status.in_([SOStatus.received, SOStatus.confirmed, SOStatus.preparing])
    ).count()
