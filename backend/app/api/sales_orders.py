...
def _so_to_dict(so):
    return {"id": so.id, "so_number": so.so_number, "customer_id": so.customer_id, "customer_name": so.customer.company_name if so.customer else "", "status": so.status, "order_date": so.order_date.isoformat() if so.order_date else None, "delivery_date": so.delivery_date.isoformat() if so.delivery_date else None, "total_amount": float(so.total_amount) if so.total_amount else 0, "currency": so.currency, "payment_terms": so.payment_terms, "destination": so.destination, "notes": so.notes, "created_at": so.created_at.isoformat() if so.created_at else None, "items": [{"id": i.id, "product_id": i.product_id, "product_name": i.product_name, "sku": i.sku, "quantity": i.quantity, "unit_price": float(i.unit_price) if i.unit_price else 0, "amount": float(i.amount) if i.amount else 0, "unit": i.unit} for i in so.items]}

@router.get("")
async def list_sales_orders(status: Optional[str] = None, db: Session = Depends(get_db)):
...
async def create_sales_order(body: dict, db: Session = Depends(get_db)):
    ...
    for item_data in body.get("items",[]):
        ...
        db.add(SalesOrderItem(id=str(uuid.uuid4()), sales_order_id=so.id, product_id=item_data.get("product_id"), product_name=item_data.get("product_name",""), sku=item_data.get("sku",""), quantity=qty, unit_price=price, amount=amount, unit=item_data.get("unit","個")))
    so.total_amount = total; db.commit(); db.refresh(so)
...
async def get_sales_order(so_id: str, db: Session = Depends(get_db)):
    ...
    if not so: raise HTTPException(404, "受注が見つかりません")
    return _so_to_dict(so)
...
async def update_so_status(so_id: str, body: dict, db: Session = Depends(get_db)):
    so = db.query(SalesOrder).filter(SalesOrder.id == so_id).first()
    if not so: raise HTTPException(404, "受注が見つかりません")
    new_status = body.get("status"); so.status = new_status; so.updated_at = datetime.utcnow()
    if new_status == SOStatus.shipped:
        for item in so.items:
            if item.product_id:
                inv = db.query(Inventory).filter(Inventory.product_id == item.product_id).first()
                if inv:
                    inv.quantity = max(0, inv.quantity - item.quantity); inv.updated_at = datetime.utcnow()
                    db.add(InventoryLog(id=str(uuid.uuid4()), inventory_id=inv.id, log_type="out", quantity_change=-item.quantity, quantity_after=inv.quantity, reference_id=so.so_number, notes=f"受注 {so.so_number} 出荷"))
...
async def delete_sales_order(so_id: str, db: Session = Depends(get_db)):
    so = db.query(SalesOrder).filter(SalesOrder.id == so_id).first()
    if not so: raise HTTPException(404, "受注が見つかりません")
    db.delete(so); db.commit(); return {"success": True}
