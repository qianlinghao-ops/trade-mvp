...
async def list_products(db: Session = Depends(get_db)):
    ...
    for p in products:
        ...
        result.append({
            ...
            "is_low_stock": (inv.quantity if inv else 0) <= p.min_stock_qty,
...
async def create_product(body: dict, db: Session = Depends(get_db)):
    product = Product(
        id=str(uuid.uuid4()),
        sku=body.get("sku", f"SKU-{uuid.uuid4().hex[:6].upper()}"),
        product_name=body.get("product_name", ""),
        product_name_en=body.get("product_name_en", ""),
        hs_code=body.get("hs_code", ""),
        unit_price=body.get("unit_price", 0),
        currency=body.get("currency", "JPY"),
        unit=body.get("unit", "個"),
        min_stock_qty=body.get("min_stock_qty", 0),
    ...
    inv = Inventory(
        ...
        quantity=body.get("initial_stock", 0),
    )
    db.add(inv)
    db.commit()
    db.refresh(product)
...
async def get_product(product_id: str, db: Session = Depends(get_db)):
    ...
    inv = db.query(Inventory).filter(Inventory.product_id == p.id).first()
    return {
        "id": p.id, "sku": p.sku, "product_name": p.product_name,
        "product_name_en": p.product_name_en, "hs_code": p.hs_code,
        "unit_price": float(p.unit_price) if p.unit_price else 0,
