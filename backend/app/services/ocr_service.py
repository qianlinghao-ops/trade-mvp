...
def _find_amount(text):
    return _find([r'(?:total|合計|総額|amount)[^\d]*([0-9,]+(?:\.\d{1,2})?)',
                  r'([0-9,]{4,}(?:\.\d{1,2})?)\s*(?:円|JPY|USD|EUR)'], text, "0")
...
def _find_items(text):
    ...
    for m in re.finditer(r'([A-Za-z0-9\-\u3040-\u9fff]+)\s+(\d+)\s*(?:個|pcs|units|本|枚|箱)?\s+([0-9,]+(?:\.\d{1,2})?)', text, re.MULTILINE):
        try:
            qty = int(m.group(2))
            price = float(re.sub(r'[^\d.]', '', m.group(3))) if re.sub(r'[^\d.]', '', m.group(3)) else 0
            items.append({"product_name": m.group(1).strip(), "quantity": qty, "unit_price": price,
                          "amount": round(qty * price, 2), "sku": "", "unit": "個"})
        except:
            continue
    if not items:
        items = [{"product_name": "（書類から自動抽出）", "quantity": 1, "unit_price": 0, "amount": 0, "sku": "", "unit": "個"}]
    return items[:10]
...
def parse_po(text):
    ...
    return {"doc_type": "po",
            ...
            "total_amount": _find_amount(text), "payment_terms": _find([r'(?:Payment|支払条件)[:\s]+([^\n]+)'], text, "T/T 30 days"),
            "items": _find_items(text), "notes": ""}
...
def parse_invoice(text):
    ...
    return {"doc_type": "invoice",
            ...
            "currency": "JPY" if re.search(r'円|JPY', text) else ("USD" if re.search(r'USD|\$', text) else "JPY"),
            "total_amount": _find_amount(text), "tax_amount": _find([r'(?:Tax|消費税|VAT)[:\s]*([0-9,]+(?:\.\d{1,2})?)'], text, "0"),
            "payment_terms": _find([r'(?:Payment|支払条件)[:\s]+([^\n]+)'], text, "T/T 30 days"),
            "items": _find_items(text), "notes": ""}

def parse_packing_list(text):
    import random
    return {"doc_type": "packing_list",
